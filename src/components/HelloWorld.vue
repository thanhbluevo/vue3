...
<div class="hello">
    <h1>{{ msg }}</h1>
    <ul>
        <li v-for="item in thanh" :key="item"> {{ item }}</li>
    </ul>

    <div v-for="(variant, index) in variants" :key="variant.id" @mouseover="updateVariant(index)" class="color-circle" :class="{ active: variant.active }" :style="{ backgroundColor : variant.color }"></div>
    ...
    <div class="product-container">
        <div class="product-image">
            <a :href="url" target="_blank">
                <img :src="image" :alt="description" :title="description" :style="imgStyle" :class="{ 'out-of-stock-img': !inStock }">
            </a>
        </div>
